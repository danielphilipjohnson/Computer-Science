$(document).ready(function () {
    ...
    function addErrorPanel(heading, message) {
      if ($(".error").length <= 0) {
        var partial =
          '<div class="w3-container w3-red  w3-card-4 error"><h3>' +
          heading +
          "</h3> <p>" +
          message +
          "</p></div> ";
    ...
    function createContainer(searchQuery, elementClass) {
      ...
      $(".search-history").append(
        "<li class='search-item'>" + searchQuery + "</li>"
      );
      var partial =
        '<div class="w3-panel w3-signal-grey w3-round w3-card-4"><h2 class="search-word">Word searched: ' +
        searchQuery.toUpperCase() +
        "</h2><p class=" +
        elementClass +
        ">Results where found for :" +
        searchQuery.toUpperCase() +
        "</p></div>";
      $(partial).prependTo("#results");
      $(".input-group").val("");
    ...
    function wikiSearch(searchItem) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          for (var result in results) {
            ...
            var resultPartial =
              ...
              page +
              pageId +
              '" target="_blank"><div class="w3-panel w3-signal-white  w3-border w3-round-large result-bar">' +
        ...
        removeErrorPanel();
        addErrorPanel(
          "Could not contact wikipedia",
          "they may have server issues or you could check your internet connection. Sorry about that."
        );
      });
    }
