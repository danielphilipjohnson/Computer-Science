...
def greedy_to_log(arr):
    inc_0_log = []
    
    for i in range(len(arr)):
    
        t = (math.log(arr[i][0]), math.log(arr[i][1]))
        inc_0_log.append(t)
...
def run_simulations():
    ...
    inc_500_log = greedy_to_log(inc_500)
    inc_1000_log = greedy_to_log(inc_1000)
    inc_2000_log = greedy_to_log(inc_2000)
    print inc_2000_log

    
    simpleplot.plot_lines("Greedy boss", 600, 600, "days", "total earnings", 
                          [inc_0_log, inc_500, inc_1000, inc_2000], False,
