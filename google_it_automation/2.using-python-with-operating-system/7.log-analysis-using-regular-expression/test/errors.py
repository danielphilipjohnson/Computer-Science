...
class TestRetrieveErrors(unittest.TestCase):
    ...
    def test_get_error_from_log(self):
        """  
        Test that ticky_check._get_error_from_log() 
        ...
        expected_value = "Tried to add information to closed ticket"
        
       
        self.assertEqual(self.ticky_check._get_error_from_log(error_input), expected_value)
    
    def test_get_error_from_log_with_no_match(self):
        """  
            Test that ticky_check._get_error_from_log() 
            returns None when there is no match
            INPUT: ticky: INFO Closed ticket [01754] (noel)
            Expected OUTPUT: 
                None   
        """
        ...
        expected_value = None
        self.assertEqual(self.ticky_check._get_error_from_log(info_input), expected_value)
    
    def test_get_error_from_log_with_empty_string(self):
        """  
            Test that ticky_check._get_error_from_log() 
        ...
        expected_value = None
        self.assertEqual(
            self.ticky_check._get_error_from_log(empty_input),             
            expected_value
        )
